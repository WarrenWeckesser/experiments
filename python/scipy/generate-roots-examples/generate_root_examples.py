...
weight function w(x).  For a given number of sample points n used in the quadrature
formula, there is a maximum degree m such that the weighted quadrature formula gives
the exact integral if the degree of p does not exceed m.
...
We'll use n=3, so the quadrature is exact for this function, and we expect good
agreement with the integral computed with `scipy.integerate.quad`.
...
Compute the integral using the quadrature rule:
...
As expected, we have good agreement between the two values:

>>> np.allclose(weighted_sum, intgrl, rtol=err/intgrl)
{allclose!r}
"""
...
for func in funcs:
    ...
    x, wts = roots_func(n, *func.param_values)
    weighted_sum = wts @ f(x)

    allclose = np.allclose(weighted_sum, q, rtol=err/q)
    ...
    text = template.format(funcname=func.funcname, propername=propername,
                           wmath=func.wmath, wfactor=wfactor, a=func.a, b=func.b,
