...
99997 FORMAT (/, ' The output was written to file sir.dat')
C
      STOP
      END
C
...
C
      STARTS = 0.9999D0
      STARTI = 1.0D0 - STARTS
      TSTART = -1.0/(RPAR(1)*STARTS)
      IF (TSTART .LT. -RPAR(2)) THEN
          WRITE(0,99900)
          WRITE(0,99910)
          STOP
      ENDIF
      ...
      ENDIF
C
C      The following bit is the old code for the initial values.
C      TSTARTFRAC = 0.01
C      TSTARTIC   = -TSTARTFRAC*RPAR(2)
C      IF (T .LT. STARTIC) THEN
C          IF (I .EQ. 1) Y = 1.0D0
C          IF (I .EQ. 2) Y = 0.0D0
C          IF (I .EQ. 3) Y = 0.0D0
C      ELSE
C          IF (I .EQ. 1) Y = STARTS + ((1.0D0-STARTS)/TSTARTIC)*T
C          IF (I .EQ. 2) Y = STARTI*(1.0D0-(T/TSTARTIC))
C          IF (I .EQ. 3) Y = 0.0D0
C      ENDIF
C
99900 FORMAT (' Error in YINIT: The start of the interval of nonzero ')
99910 FORMAT (' I(t) is less than -h ')
      RETURN
      END
C
C
...
C
      WRITE (LOUT,99999) T, Y(1), Y(2), Y(3)
C
...
C      ENDIF
C
99999 FORMAT (' ', D15.5, D15.5, D15.5, D15.5)
